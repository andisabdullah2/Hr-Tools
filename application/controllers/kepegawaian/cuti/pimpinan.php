...
class pimpinan extends ApplicationBase {
    ...
    // my flow
    private $now_flow_id = '11002';
    private $next_flow_id = '11003';
    private $prev_flow_id = '11001';

    public function __construct() {
        // parent constructor
        parent::__construct();
        // load model
        $this->load->model("kepegawaian/cuti/m_pimpinan");
        //load library
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "kepegawaian/cuti/pimpinan/list.html");
        // get tahun
        ...
        /* start of pagination --------------------- */
        // pagination
        $config['base_url'] = site_url("kepegawaian/cuti/pimpinan/index/");
    ...
    public function proses_cari() {
        ...
        // redirect
        redirect("kepegawaian/cuti/pimpinan");
    }
    ...
    public function approval($process_id = "") {
        ...
        // set template content
        $this->smarty->assign("template_content", "kepegawaian/cuti/pimpinan/approval.html");
        // get detail data
    ...
    public function approval_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {            
            ...
            $data_cuti = $this->m_pimpinan->get_jenis_and_tahun_cuti_by_id($cuti_id);
            $params = array(
                $data_cuti['user_id'],
                $data_cuti['jenis_id'],
                $data_cuti['tahun'],
            );
            $total_jatah_cuti = $this->m_pimpinan->get_total_jatah_cuti($params);
            $total_cuti_terpakai = $this->m_pimpinan->get_total_cuti_terpakai($params);
            $total_cuti_pengajuan = $this->m_pimpinan->get_total_cuti_pengajuan($cuti_id);
            $sisa_cuti = $total_jatah_cuti - $total_cuti_terpakai;
            //hitung
            if ($total_cuti_pengajuan > $sisa_cuti && $process_st == 'approve'){
                ...
                redirect("kepegawaian/cuti/pimpinan/approval/" . $cuti_id);
            }
            //update flow_now
            $params = array(
            ...
            if ($process_st == 'approve') {
                ...
                if($this->m_pimpinan->check_if_have_references($this->input->post('process_id',true))){
                    ...
                    $references_id = $this->m_pimpinan->get_process_reference($cuti_id);
                }                    
                $params = array(
                ...
                $this->m_pimpinan->update_cuti_st($params, $where);
                // update flow pegawai_cuti_process
                $params = array(
                'process_st' => 'reject',
                'action_st' => 'done',
                'mdb_finish' => $this->com_user['user_id'],
                ...
                $this->m_pimpinan->update_cuti_st_cuti_process($params, $where);
                ...
                $params = array(
                    ...
                    'process_references_id' => $this->m_pimpinan->get_process_reference($cuti_id),
                    'action_st' => 'process', 
                    'process_st' => 'waiting', 
                    'mdb' => $this->com_user['user_id'],
                    'mdb_name' => $this->com_user['user_alias'], 
        ...
        // default
        redirect('kepegawaian/cuti/pimpinan/approval/' . $this->input->post('process_id'));
    }    
}
