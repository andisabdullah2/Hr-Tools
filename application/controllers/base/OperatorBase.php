...
class ApplicationBase extends CI_Controller {
    ...
    protected function base_view_app() {
        ...
        // get user login
        $session = $this->tsession->userdata('session_id');
        if (!empty($session)) {
            // get com user
        ...
        // get date now
        $hari_ini = $this->datetimemanipulation->get_date_now();
        $this->smarty->assign("hari_ini", $hari_ini);
        // display global link
    ...
    protected function _display_top_navigation() {
        ...
        if ($rs_id) {
            ...
            foreach ($rs_id as $rec) {
                ...
                // data
                $html .= '<a class="btn btn-sm text-dark mr-1 mb-2 ' . $parent_active . '" style="width: 30px;" title="' . $rec['nav_title'] . '" data-toggle="tab" data-target="#' . $rec['nav_id'] . '"><i class="' . $rec['nav_icon'] . '"></i></a>';
            }
        }
        // output
        $this->smarty->assign("list_top_nav", $html);
    }
    ...
    protected function _display_sidebar_navigation() {
        ...
        if ($rs_tab) {
            foreach ($rs_tab as $tab) {
                ...
                // html
                $html .= '<div class="tab-pane ' . $parent_active . '" id="' . $tab['nav_id'] . '" role="tabpanel">';
