...
class dirut extends ApplicationBase {

    // set 
    protected $group_id = 24;
    protected $flow_id = 24004;
    ...
    public function __construct() {
        ...
        parent::__construct();
        // load model
        $this->load->model('keuangan/pembelian_lebih_satu_juta/m_approval');
        // load library
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "keuangan/pembelian_lebih_satu_juta/dirut/index.html");
        // search
        $search = $this->tsession->userdata("pembelian_lebih_satu_juta_dirut_search");
        ...
        // pagination
        $config['base_url'] = site_url("keuangan/pembelian_lebih_satu_juta/dirut/index/");
        $config['total_rows'] = $this->m_approval->get_total_advance($params);
        ...
        $this->smarty->assign('rs_id', $this->m_approval->get_list_advance($params));
    ...
    public function search_process(){
        ...
        if ($this->input->post('save') == "Cari") {
            ...
            // set session search
            $this->tsession->set_userdata("pembelian_lebih_satu_juta_dirut_search", $params);
        } else{
            // unset session search
            $this->tsession->unset_userdata("pembelian_lebih_satu_juta_dirut_search");
        }
        // default redirect
        redirect("keuangan/pembelian_lebih_satu_juta/dirut");
    }
    ...
    public function detail($trx_id = ''){
        ...
        // set template content
        $this->smarty->assign("template_content", "keuangan/pembelian_lebih_satu_juta/dirut/detail.html");
        // load style
        ...
        // get data
        $detail = $this->m_approval->get_trx_advance_by_id(array($this->group_id, $this->flow_id, $trx_id));
        // check data
        if (empty($detail)) {
            // default error
        ...
        $this->smarty->assign("rs_id", $this->m_approval->get_list_rincian_item_pembelian($trx_id));
        // get list advance process
        $this->smarty->assign('rs_process', $this->m_approval->get_list_advance_process_by_trx_id(array($trx_id,$this->group_id)));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // output
    ...
    public function pengajuan_process(){
        ...
        // get data process
        $now_flow = $this->m_approval->get_trx_advance_process_by_id($process_id);
        ...
        if ($this->tnotification->run() == TRUE) {
            ...
            if (!$this->m_approval->update_trx_advance_process($params, $where)) {
                // error update
                $this->tnotification->sent_notification('error', 'Persetujuan pengajuan gagal dilakukan!');
                redirect('keuangan/pembelian_lebih_satu_juta/dirut/detail/' . $trx_id);
            }

            // get new process id
            $new_process_id = $this->m_approval->get_id();
            // check process status
            if ($process_st == 'approve') {
                // get next flow id
                $next_flow = $this->m_approval->get_next_flow_id_by_trx_id($trx_id);
                // check flow id
