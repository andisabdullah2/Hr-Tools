...
class pengembalian extends ApplicationBase {

    // set 
    protected $group_id = 23;
    protected $flow_id = 23007;
    ...
    public function __construct() {
        ...
        parent::__construct();
        // load model
        $this->load->model('keuangan/pembelian_kurang_satu_juta/m_pengembalian');
        // load library
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "keuangan/pembelian_kurang_satu_juta/pengembalian/index.html");
        // search
        $search = $this->tsession->userdata("pembelian_kurang_satu_juta_pengembalian_search");
        ...
        // pagination
        $config['base_url'] = site_url("keuangan/pembelian_kurang_satu_juta/pengembalian/index/");
        $config['total_rows'] = $this->m_pengembalian->get_total_advance($params);
        ...
        $this->smarty->assign('rs_id', $this->m_pengembalian->get_list_advance($params));
    ...
    public function search_process(){
        ...
        if ($this->input->post('save') == "Cari") {
            ...
            // set session search
            $this->tsession->set_userdata("pembelian_kurang_satu_juta_pengembalian_search", $params);
        } else{
            // unset session search
            $this->tsession->unset_userdata("pembelian_kurang_satu_juta_pengembalian_search");
        }
        // default redirect
        redirect("keuangan/pembelian_kurang_satu_juta/pengembalian");
    }
    ...
    public function detail($trx_id = ''){
        ...
        // set template content
        $this->smarty->assign("template_content", "keuangan/pembelian_kurang_satu_juta/pengembalian/detail.html");
        // load style
        ...
        // get data
        $detail = $this->m_pengembalian->get_trx_advance_by_id(array($this->group_id, $this->flow_id, $trx_id));
        // check data
        if (empty($detail)) {
            // default error
        ...
        $this->smarty->assign("rs_id", $this->m_pengembalian->get_list_rincian_item_pembelian($trx_id));
        // get list lpj
        $this->smarty->assign("rs_lpj", $this->m_pengembalian->get_list_lpj_by_trx_id($trx_id));
        // get list advance process
        $this->smarty->assign('rs_process', $this->m_pengembalian->get_list_advance_process_by_trx_id(array($trx_id,$this->group_id)));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // output
    ...
    public function pengajuan_process(){
        ...
        // get data process
        $now_flow = $this->m_pengembalian->get_trx_advance_process_by_id($process_id);
        ...
        if ($this->tnotification->run() == TRUE) {
            ...
            if (!$this->m_pengembalian->update_trx_advance_process($params, $where)) {
                // error update
                $this->tnotification->sent_notification('error', 'Persetujuan pengajuan gagal dilakukan!');
                redirect('keuangan/pembelian_kurang_satu_juta/pengembalian/detail/' . $trx_id);
            }
            ...
            if ($process_st == 'approve') {
                ...
                // params
                $params = array(
                    'process_id' => $this->m_pengembalian->get_id(),
                    'trx_id' => $trx_id,
                ...
                $this->m_pengembalian->insert_trx_advance_process($params);                
            }
            ...
            // update trx advance
            $this->m_pengembalian->update_trx_advance($params,$where);
