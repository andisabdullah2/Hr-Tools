...
class keuangan extends ApplicationBase {
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "keuangan/advance_jaldin/keuangan/index.html");
        // load style
        ...
        $search = $this->tsession->userdata('search_advance_jaldin_keuangan') ? $this->tsession->userdata('search_advance_jaldin_keuangan') : '';
        if (!empty($search)) {
        ...
        /* start of pagination --------------------- */
        // pagination
        $config['base_url'] = site_url("keuangan/advance_jaldin/keuangan/index/");
        $config['total_rows'] = $this->m_approval->get_total_advance_jaldin(array(3, $item_uraian));
        ...
        /* end of pagination ---------------------- */
        // get data
        $params = array(3, $item_uraian, ($start - 1), $config['per_page']);
    ...
    public function search_process() {
        ...
        if ($this->input->post('save') == "Cari") {
            ...
            // set session
            $this->tsession->set_userdata("search_advance_jaldin_keuangan", $params);
        } else {
            // unset session
            $this->tsession->unset_userdata("search_advance_jaldin_keuangan");
        }
        // redirect
        redirect("keuangan/advance_jaldin/keuangan");
    }
    ...
    public function detail($process_id="") {
        ...
        // set template content
        $this->smarty->assign("template_content", "keuangan/advance_jaldin/keuangan/detail.html");
        // load style
        ...
        $this->smarty->assign("rs_flow", $this->m_approval->get_list_flow_plan(array($detail['trx_id'], 22, 3)));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // output
    ...
    public function pengajuan_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            switch ( $this->input->post('process_st', TRUE) ) {
                case 'approve':
                    // flow 4
                    $next_flow = $this->m_approval->get_flow_by_params(array(22, 4));
                    if ( $next_flow ) {
