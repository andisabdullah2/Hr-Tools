...
class hrd extends ApplicationBase {
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "keuangan/advance_jaldin/hrd/index.html");
        // load style
        ...
        $search = $this->tsession->userdata('search_advance_jaldin_hrd') ? $this->tsession->userdata('search_advance_jaldin_hrd') : '';
        if (!empty($search)) {
        ...
        /* start of pagination --------------------- */
        // pagination
        $config['base_url'] = site_url("keuangan/advance_jaldin/hrd/index/");
        $config['total_rows'] = $this->m_approval->get_total_advance_jaldin(array(2, $item_uraian));
        ...
        /* end of pagination ---------------------- */
        // get data
        $params = array(2, $item_uraian, ($start - 1), $config['per_page']);
    ...
    public function search_process() {
        ...
        if ($this->input->post('save') == "Cari") {
            ...
            // set session
            $this->tsession->set_userdata("search_advance_jaldin_hrd", $params);
        } else {
            // unset session
            $this->tsession->unset_userdata("search_advance_jaldin_hrd");
        }
        // redirect
        redirect("keuangan/advance_jaldin/hrd");
    }
    ...
    public function detail($process_id="") {
        ...
        // set template content
        $this->smarty->assign("template_content", "keuangan/advance_jaldin/hrd/detail.html");
        // load style
        ...
        $this->smarty->assign("rs_flow", $this->m_approval->get_list_flow_plan(array($detail['trx_id'], 22, 2)));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // output
    ...
    public function pengajuan_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            switch ( $this->input->post('process_st', TRUE) ) {
                case 'approve':
                    // flow 3
                    $next_flow = $this->m_approval->get_flow_by_params(array(22, 3));
                    if ( $next_flow ) {
