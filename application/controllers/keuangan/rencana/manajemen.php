...
class manajemen extends ApplicationBase {
    ...
    public function __construct() {
        ...
        parent::__construct();
        // load model
        $this->load->model('keuangan/rencana/m_manajemen');
        // load library
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "keuangan/rencana/manajemen/index.html");
        // search
        $search = $this->tsession->userdata("finance_manajemen_search");
        // search parameter
        ...
        // pagination
        $config['base_url'] = site_url("keuangan/rencana/manajemen/index/");
        $config['total_rows'] = $this->m_manajemen->get_total_plan_manajemen($params);
        ...
        // get data 
        $this->smarty->assign("rs_id", $this->m_manajemen->get_list_plan_manajemen($params));
        // get all tahun
        $this->smarty->assign("rs_tahun", $this->m_manajemen->get_all_tahun_project());
        // get all unit kerja
        $this->smarty->assign("rs_unit_kerja", $this->m_manajemen->get_list_unit_kerja());
        // notification
    ...
    public function search_process() {
        ...
        if ($this->input->post('save') == "Reset") {
            ...
            $this->tsession->unset_userdata("finance_manajemen_search");
        } else {
            // params
            $params = array(
            ...
            // set session search
            $this->tsession->set_userdata("finance_manajemen_search", $params);
        }
        // default redirect
        redirect("keuangan/rencana/manajemen");
    }
    ...
    public function add(){
        ...
        // set template content
        $this->smarty->assign("template_content", "keuangan/rencana/manajemen/add.html");
        // get all project rka manajemen
        $this->smarty->assign("rs_projects", $this->m_manajemen->get_all_project_rka_manajemen());
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // output
    ...
    public function add_process(){
        ...
        if ($this->tnotification->run() == TRUE) {
            // get last plan id
            $plan_id = $this->m_manajemen->get_last_plan_id(date('Ym'));
            // params
            $params = array(
                'plan_id' => $plan_id,
            ...
            // insert
            if (!$this->m_manajemen->insert_projects_budget_plan($params)) {
                // error insert
                $this->tnotification->sent_notification("error", "Data rencana gagal diproses!");
                redirect("keuangan/rencana/manajemen/add");
            }
            // process
            $params = array(
                'process_id' => $this->m_manajemen->get_id(),
                'plan_id' => $plan_id,
                'flow_id' => $this->m_manajemen->get_first_flow_by_group_id('15'),
                'process_st' => 'waiting',
                'action_st' => 'process',
                'mdb' => $this->com_user['user_id'],
                'mdb_name' => $this->com_user['user_alias'],
            ...
            // insert process
            $this->m_manajemen->insert_projects_budget_process($params);
            // success insert
            $this->tnotification->delete_last_field();
            $this->tnotification->sent_notification("success", "Data rencana berhasil diproses!");
            // redirect
            redirect("keuangan/rencana/manajemen/item/" . $plan_id);
        } else {
            // default error
    ...
    public function edit($plan_id = ""){
        ...
        // set template content
        $this->smarty->assign("template_content", "keuangan/rencana/manajemen/edit.html");
        // get data
        $detail = $this->m_manajemen->get_data_plan_by_id($plan_id);
        // check data plan
        if (empty($detail)) {
            // no data
    ...
    public function edit_process(){
        ...
        if ($this->tnotification->run() == TRUE) {
            ...
            // update
            if (!$this->m_manajemen->update_projects_budget_plan($params,$where)) {
                // error update
                $this->tnotification->sent_notification("error", "Data rencana gagal diproses!");
                redirect("keuangan/rencana/manajemen/edit/" . $plan_id);
            }
            // success update
            $this->tnotification->delete_last_field();
            $this->tnotification->sent_notification("success", "Data rencana berhasil diproses!");
    ...
    public function delete($plan_id = ''){
        ...
        // set template content
        $this->smarty->assign("template_content", "keuangan/rencana/manajemen/delete.html");
        // get data
        $detail = $this->m_manajemen->get_data_plan_by_id($plan_id);
        // check data plan
        if (empty($detail)) {
    ...
    public function delete_process(){
        ...
        if ($this->tnotification->run() == TRUE) {
            ...
            $where = array('plan_id' => $plan_id);
            // delete
            if (!$this->m_manajemen->delete_projects_budget_plan($where)) {
                // error delete
                $this->tnotification->sent_notification('error', 'Data pengajuan RKA Manajemen gagal dihapus!');
                redirect('keuangan/rencana/manajemen/delete/' . $plan_id);
            }
            // success delete
            $this->tnotification->delete_last_field();
            $this->tnotification->sent_notification("success", "Data pengajuan RKA Manajemen berhasil dihapus!");
            // redirect
            redirect("keuangan/rencana/manajemen");
        } else {
            // default error
    ...
    public function review($plan_id = ""){
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "keuangan/rencana/manajemen/review.html");
        // get data
        $detail = $this->m_manajemen->get_data_plan_by_id($plan_id);
        // check data plan
        if (empty($detail)) {
        ...
        $this->smarty->assign("rs_item", $this->m_manajemen->get_list_rencana_item_by_plan_id(array($plan_id)));
        // get list detail
        $this->smarty->assign("rs_detail", $this->m_manajemen->get_list_item_and_detail_by_plan_id($plan_id));
        // get list plan process
        $this->smarty->assign("rs_process", $this->m_manajemen->get_list_plan_process_by_plan_id($plan_id));
        // notification
        $this->tnotification->display_notification();
        // output
        parent::display();
    ...
    public function item($plan_id = ""){
        ...
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "keuangan/rencana/manajemen/item.html");
        // get data
        $detail = $this->m_manajemen->get_data_plan_by_id($plan_id);
        // check data plan
        if (empty($detail)) {
        ...
        $this->smarty->assign("detail", $detail);
        // list perusahaan
        $this->smarty->assign("rs_perusahaan", $this->m_manajemen->get_list_perusahaan());
        // list group project
        $this->smarty->assign("rs_project_group", $this->m_manajemen->get_list_project_group());
        // get list rencana item
        $this->smarty->assign("rs_id", $this->m_manajemen->get_list_rencana_item_by_plan_id(array($plan_id)));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // output
    ...
    public function item_add_process(){
        ...
        if ($this->tnotification->run() == TRUE) {
            // get item id
            $item_id = $this->m_manajemen->get_last_item_id($plan_id);
            // get item no
            $item_no = $this->m_manajemen->get_item_no_rencana_item_by_plan_id($plan_id);
            // input
            ...
            // insert
            if (!$this->m_manajemen->insert_projects_budget_item($params)) {
                // error insert
            ...
            $where = array('plan_id' => $plan_id);
            // update budget plan
            $this->m_manajemen->update_projects_budget_plan($params,$where);
    ...
    public function item_edit_process(){
        ...
        if ($this->tnotification->run() == TRUE) {
            ...
            // update
            if (!$this->m_manajemen->update_projects_budget_item($params,$where)) {
                // error update
                $this->tnotification->sent_notification("error", "Data rencana item gagal diproses!");
                redirect("keuangan/rencana/manajemen/item/" . $plan_id);
            }
            // get data 
            $detail = $this->m_manajemen->get_data_rencana_item_by_id($item_id);
    ...
    public function delete_item(){
        ...
        if ($this->tnotification->run() == TRUE) {
            // get data
            $result = $this->m_manajemen->get_data_rencana_item_by_id($item_id);
            // params
            $params_plan = array(
            ...
            // delete process
            if (!$this->m_manajemen->delete_projects_budget_item($where)) {
                // error delete
                $this->tnotification->sent_notification('error', 'Data rencana item gagal dihapus!');
                redirect('keuangan/rencana/manajemen/item/' . $plan_id);
            }
            // update projects budget plan
            $this->m_manajemen->update_projects_budget_plan($params_plan,$where_plan);
            // success
            $this->tnotification->delete_last_field();
    ...
    public function detail($plan_id = ''){
        ...
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "keuangan/rencana/manajemen/detail.html");
        // get data
        $detail = $this->m_manajemen->get_data_plan_by_id($plan_id);
        // check data plan
        if (empty($detail)) {
        ...
        $this->smarty->assign("detail", $detail);
        // get list rencana item
        $this->smarty->assign("rs_id", $this->m_manajemen->get_list_item_and_detail_by_plan_id($plan_id));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // output
    ...
    public function detail_add_process(){
        ...
        if ($this->tnotification->run() == TRUE) {
            // get detail id
            $detail_id = $this->m_manajemen->get_last_detail_id($item_id);
            // get detail no
            $detail_no = $this->m_manajemen->get_detail_no_rencana_detail_by_item_id($item_id);
            // params
            $params = array(
                'detail_id' => $detail_id,
                'item_id' => $item_id,
            ...
            // insert
            if (!$this->m_manajemen->insert_projects_budget_detail($params)) {
                // error insert
                $this->tnotification->sent_notification("error", "Data rencana detail item gagal diproses!");
                redirect("keuangan/rencana/manajemen/detail/" . $plan_id);
            }           
    ...
    public function detail_edit_process(){
        ...
        if ($this->tnotification->run() == TRUE) {
            ...
            // update
            if (!$this->m_manajemen->update_projects_budget_detail($params,$where)) {
                // error update
                $this->tnotification->sent_notification("error", "Data rencana detail item gagal diproses!");
                redirect("keuangan/rencana/manajemen/detail/" . $plan_id);
            }
    ...
    public function delete_detail(){
        ...
        if ($this->tnotification->run() == TRUE) {
            ...
            // delete process
            if (!$this->m_manajemen->delete_projects_budget_detail($where)) {
                // error delete
                $this->tnotification->sent_notification('error', 'Data rencana detail item gagal diproses!');
                redirect('keuangan/rencana/manajemen/detail/' . $detail_id);
            }
            // success
            $this->tnotification->delete_last_field();
    ...
    public function output($plan_id = ''){
        ...
        $this->_set_page_rule("C");
        // set template content
        $this->smarty->assign("template_content", "keuangan/rencana/manajemen/output.html");
        // get data
        $detail = $this->m_manajemen->get_data_plan_by_id($plan_id);
        // check data plan
        if (empty($detail)) {
        ...
        $this->smarty->assign("detail", $detail);
        // data jenis output
        $this->smarty->assign("rs_js_output", $this->m_manajemen->get_list_jenis_output());
        // data jenis kegiatan
        $this->smarty->assign("rs_js_kegiatan", $this->m_manajemen->get_list_jenis_kegiatan());
        // data kegiatan
        $rs_kegiatan = $this->m_manajemen->get_list_rencana_program(array($detail['struktur_cd'],$detail['jenis_kode_kegiatan']));
    ...
    public function output_process(){
        ...
        if ($this->tnotification->run() == TRUE) {
            ...
            if (empty($kode_output)) {
                // insert
                // get kode output new
                $kode_output = $this->m_manajemen->get_last_kode_output($kode_kegiatan);
                // array
                $params['kode_output'] = $kode_output;
                // insert
                if (!$this->m_manajemen->insert_rencana_output($params)) {
                    // error insert
                    $this->tnotification->sent_notification("error", "Data rencana output gagal diproses!");
                    redirect("keuangan/rencana/manajemen/output/" . $plan_id);
                }
                ...
                if (!$this->m_manajemen->update_projects_budget_plan($params_plan,$where_plan)) {
                    // error insert
                    $this->tnotification->sent_notification("error", "Data rencana output gagal diproses!");
                    redirect("keuangan/rencana/manajemen/output/" . $plan_id);
                }
                ...
                // update
                // check data
                if (!$this->m_manajemen->is_exist_rencana_output_by_plan_id($plan_id)) {
                    // no data
                ...
                // insert
                if (!$this->m_manajemen->update_rencana_output($params,$where)) {
                    // error insert
                    $this->tnotification->sent_notification("error", "Data rencana output gagal diproses!");
                    redirect("keuangan/rencana/manajemen/output/" . $plan_id);
                }
    ...
    public function verifikasi_process($item_id = ''){
        ...
        $this->_set_page_rule("U");
        // get nilai data
        $result = $this->m_manajemen->get_nilai_by_item_id($item_id);
        // check data
        if (empty($result)) {
            // no data
        ...
        // update item
        $this->m_manajemen->update_projects_budget_item($params_item,$where_item);
        ...
        // redirect
        redirect('keuangan/rencana/manajemen/detail/' . $result['plan_id']);
    }
    ...
    public function send_process(){
        ...
        $plan_id = $this->input->post('plan_id',true);
        // check data
        if(!$this->m_manajemen->is_exist_plan_projects_by_plan_id($plan_id)){
            // no data
        ...
        if ($this->tnotification->run() == TRUE) {
            ...
            // get data process
            $process = $this->m_manajemen->get_next_flow_id_by_plan_id($plan_id);
            ...
            // update process
            $this->m_manajemen->update_projects_budget_process($params,$where);
            // insert process next
            $params = array(
                'process_id' => $this->m_manajemen->get_id(),
                'plan_id' => $plan_id,
            ...
            // success
            $this->tnotification->delete_last_field();
            $this->tnotification->sent_notification("success", "Data pengajuan RKA Manajemen berhasil dikirim!");
            // redirect
            redirect("keuangan/rencana/manajemen");
        } else {
            // default error
            $this->tnotification->sent_notification('error', 'Data pengajuan RKA Manajemen gagal diproses!');
        }
        // default redirect
        redirect('keuangan/rencana/manajemen/output/' . $plan_id);
    }
    ...
    function list_akun(){
        ...
        // get data
        $rs_akun = $this->m_manajemen->get_list_akun_by_perusahaan_id($perusahaan_id);
        // check
        if (!empty($rs_akun)) {
            // ada data
    ...
    function list_kegiatan(){
        ...
        // get data
        $rs_kegiatan = $this->m_manajemen->get_list_rencana_program(array($struktur_cd,$jenis_kode_kegiatan));
        // check
