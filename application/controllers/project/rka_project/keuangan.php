...
class keuangan extends ApplicationBase {
    ...
    public function index() {
        ...
        $this->smarty->assign("template_content", "project/rka_project/keuangan/index.html");
        // load style
        ...
        // search
        $search = $this->tsession->userdata("approval_keuangan_rka_project_search");
        // search parameters
        ...
        /* start of pagination --------------------- */
        // pagination
        $config['base_url'] = site_url("project/rka_project/keuangan/index/");
        $params = array($this->flow_keuangan, 16,$project,$project,$process_st);
        ...
        /* end of pagination ---------------------- */
        // get list
        $params = array($this->flow_keuangan, 16,$project,$project,$process_st, ($start - 1), $config['per_page']);
    ...
    public function search_process() {
        ...
        if ($this->input->post('save') == "Reset") {
            $this->tsession->unset_userdata('approval_keuangan_rka_project_search');
        } else {
            $params = array(
                "project" => $this->input->post("project",true),
                "process_st" => $this->input->post("process_st",true)
            );
            $this->tsession->set_userdata("approval_keuangan_rka_project_search", $params);
        }
        // redirect
        redirect("project/rka_project/keuangan");
    }
    ...
    public function detail($process_id = '')
    {
        ...
        // set template content
        $this->smarty->assign("template_content", "project/rka_project/keuangan/detail.html");
        // load style
        ...
        // params
        $params= array($process_id,$this->flow_keuangan, 16);
        ...
        $this->smarty->assign('flow_pimpinan', $this->m_approval->get_detail_flow_by_plant(array($detail['plan_id'], $this->flow_pimpinan, 16)));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // output
    ...
    public function proses_persetujuan()
    {
        ...
        if($this->tnotification->run() != false){
            // params
            $params = array($this->input->post('process_id', TRUE),$this->flow_keuangan, 16);
            // get detail project
            $detail = $this->m_approval->get_detail_flow($params);
            // cek data project plan
            if(empty($detail)){
            ...
            // set params update keuangan
            $params_keuangan = array(
            ...
            $where_keuangan = array(
                'process_id' => $this->input->post('process_id', TRUE)
            );
            // cek process
            if ($this->input->post('process') == "approve") {
                // update params update dirut
                $params_keuangan['process_st'] = 'approve';
                // get flow dirut
                $flow_dirut = $this->m_approval->get_detail_flow_by_plant( array($detail['plan_id'],$this->flow_dirut, 16));
                ...
                if(!$flow_dirut){
                    ...
                    $params = array(
                        ...
                        'flow_id' => $this->flow_dirut,
                        'flow_revisi_id' => $this->flow_keuangan,
                ...
                // update params update dirut
                $params_keuangan['process_st'] = 'reject';
                // update project plan
                // set params
                $params = array(
                    'send_status' => 'done',
            ...
            // update project proses keuangan
            if($this->m_approval->update_project_budget_process($params_keuangan, $where_keuangan)){
                // notification
        ...
        // default redirect
        redirect("project/rka_project/keuangan/detail/". $this->input->post('process_id', TRUE));
    }
