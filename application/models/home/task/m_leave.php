...
class m_leave extends CI_Model {
    ...
    */  
    ...
    // insert leave    
    public function insert_leave($params) {
    ...
    // get detail leave
    function get_detail_leave($leave_id) {
        $sql = "SELECT a.*, b.nama_lengkap, c.jenis_cuti
                FROM pegawai_cuti a
                INNER JOIN pegawai b ON a.user_id = b.user_id
                INNER JOIN data_jenis_cuti c ON a.jenis_id=c.jenis_id
                WHERE a.cuti_id = ?";
        $query = $this->db->query($sql, $leave_id);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
            $query->free_result();
            return $result;
    ...
    // get cetak leave 
    function get_cetak_leave_by_id($params) {
    ...
    // update_leave 
    function update_leave($params, $where) {
        return $this->db->update('pegawai_cuti', $params, $where);
    }     
    
    // update leave st
    function update_leave_st($params, $where) {
        return $this->db->update('pegawai_cuti', $params, $where);
    }
    ...
    */  
    ...
    // get leave_st by leave_id
    function get_leave_st($leave_id){
