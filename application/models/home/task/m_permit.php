   ...
   // get total permit
    function get_total_permit($params) {
        $sql = "SELECT COUNT(a.izin_id)'total'
                ...
                WHERE b.nama_lengkap LIKE ? AND YEAR(a.izin_tanggal) = ? AND (izin_status = 'draft' OR izin_status = 'rejected') 
                AND f.flow_id = '12001' AND a.user_id LIKE ?";
    ...
    function get_all_permit_by_limit($params) {
        $sql = "SELECT a.*, b.nama_lengkap, c.jenis_izin, f.process_st, f.flow_id,f.action_st
                ...
                AND (izin_status = 'draft' OR izin_status = 'rejected') 
                AND f.flow_id = '12001'
                AND a.user_id LIKE ?
                ORDER BY a.mdd ASC
                LIMIT ?, ?";
    ...
    function get_task_flow_id($task_number) {
        $sql = "SELECT flow_id FROM task_flow WHERE group_id='12' AND task_number LIKE ?";
    ...
    function get_detail_permit($permit_id){
        $sql = "SELECT a.*, b.nama_lengkap, c.jenis_izin
                FROM pegawai_izin a
                INNER JOIN pegawai b ON a.user_id = b.user_id
                INNER JOIN data_jenis_izin c ON a.jenis_id=c.jenis_id
                WHERE a.izin_id = ?";
