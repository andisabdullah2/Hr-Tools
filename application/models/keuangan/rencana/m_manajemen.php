...
class m_manajemen extends CI_Model {
    ...
    // get total plan manajemen
    function get_total_plan_manajemen($params){
    	...
    	$sql = "SELECT COUNT(*) as total 
				...
				WHERE b.`jenis_kode_kegiatan` = 'A' AND YEAR(b.`project_start`) LIKE ?
    ...
    // get list plan manajemen
    function get_list_plan_manajemen($params){
    ...
    function get_data_plan_by_id($params){
    	$sql = "SELECT a.*, b.`project_name`, b.project_alias, b.struktur_cd, c.kode_output, c.kode_kegiatan, 
				...
				WHERE b.jenis_kode_kegiatan = 'A' AND a.`plan_id` = ?";
		$query = $this->db->query($sql,$params);
		if ($query->num_rows() > 0) {
			$result = $query->row_array();
			$query->free_result();
    ...
    function get_list_rencana_item_by_plan_id($params){
    	...
    	$sql = "SELECT a.*, b.`group_title`, c.`nama_akun`, c.`kode_akun_alias`, d.`perusahaan_nama`, 
				...
				WHERE f.`jenis_kode_kegiatan` = 'A' AND a.`plan_id` = ?
    ...
    function get_data_rencana_item_by_id($params){
    	$sql = "SELECT a.*, b.`nilai_biaya`, d.`group_title`, e.`kode_akun_alias`, e.`nama_akun`, f.`perusahaan_nama`, 
				...
				WHERE c.`jenis_kode_kegiatan` = 'A' AND a.item_id = ?";
		$query = $this->db->query($sql,$params);
		if ($query->num_rows() > 0) {
			$result = $query->row_array();
			$query->free_result();
    ...
    function get_all_project_rka_manajemen(){
