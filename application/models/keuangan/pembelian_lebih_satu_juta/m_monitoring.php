...
class m_monitoring extends CI_Model {
    ...
    function get_total_advance($params){
    	$param = array($params[0], $params[1], $params[1]);
    	$sql = "SELECT COUNT(*) as total
				...
				WHERE a.group_id = ? AND (a.advance_uraian LIKE ? OR b.item_uraian LIKE ?)";
    ...
    function get_list_advance($params){
    	$param = array($params[0], $params[1], $params[1], $params[2], $params[3]);
    ...
    function get_trx_advance_by_id($params) {
        $sql = "SELECT a.*, b.group_name, c.kode_output, c.item_no, c.item_uraian, d.struktur_nama, d.struktur_singkatan,
        		IFNULL(total,0) AS total_rincian, IFNULL(debit,0) AS total_debit, 
    ...
    function get_list_advance_process_by_trx_id($params){
    	$sql = "SELECT a.*, b.`flow_id` AS process_flow_id, b.process_st, c.advance_status, c.send_by_name, c.send_date, 
