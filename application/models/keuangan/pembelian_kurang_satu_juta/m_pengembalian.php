...
class m_pengembalian extends CI_Model {
    ...
    function get_total_advance($params){
    	...
    	$sql = "SELECT COUNT(*) as total
				...
				WHERE d.process_st <> 'approve' AND a.`group_id` = ? AND e.flow_id = ? 
                AND (a.advance_uraian LIKE ? OR b.item_uraian LIKE ?)";
    ...
    function get_trx_advance_by_id($params) {
        $sql = "SELECT a.*, b.group_name, c.kode_output, c.item_no, c.item_uraian, d.struktur_nama, d.struktur_singkatan,
        		IFNULL(total,0) AS total_rincian, e.process_id, e.`process_st`,
        		IFNULL(debit,0) AS total_debit, 
                IFNULL(kredit,0) AS total_kredit, (IFNULL(kredit,0)-IFNULL(debit,0)) AS sisa,
                CASE WHEN j.process_st = 'waiting' THEN task_label_waiting
					WHEN j.process_st = 'approve' THEN task_label_approve
					WHEN j.process_st = 'reject' THEN task_label_reject
					END AS label,
				j.process_st as last_process_st, j.catatan, j.mdb_finish_name, j.mdd_finish
                FROM trx_advance a
                ...
                INNER JOIN task_flow f ON f.flow_id = e.flow_id
                LEFT JOIN (
                ...
                LEFT JOIN trx_advance_process i ON i.process_id = e.process_references_id
                LEFT JOIN trx_advance_process j ON j.process_id = i.process_references_id AND j.flow_id <> '23005'
