...
class m_approval extends CI_Model {
    ...
    // get_total_advance_jaldin
    function get_total_advance_jaldin($params) {
        $sql = "SELECT COUNT(a.trx_id) 'total'
                ...
                INNER JOIN (
                    ...
                    WHERE b.group_id = '22' AND b.task_number = ?
                    GROUP BY a.trx_id
                ...
                WHERE b.item_uraian LIKE ? AND e.process_st = 'waiting' AND a.group_id = '22'
    ...
    // get_list_advance_jaldin
    function get_list_advance_jaldin($params) {
        $sql = "SELECT b.item_uraian, c.struktur_nama, c.struktur_singkatan, a.advance_tanggal, a.advance_total_requested, a.trx_id, e.process_st, e.process_id, a.advance_total_approved, f.uraian_tugas
                FROM trx_advance a 
                INNER JOIN rencana_item b ON a.kode_item = b.kode_item
                INNER JOIN data_struktur_organisasi c ON a.struktur_cd = c.struktur_cd
                INNER JOIN (
    ...
    // get_total_advance_jaldin_lpj
    function get_total_advance_jaldin_lpj($params) {
        $sql = "SELECT COUNT(a.trx_id) 'total'
                FROM trx_advance a 
                INNER JOIN rencana_item b ON a.kode_item = b.kode_item
                INNER JOIN data_struktur_organisasi c ON a.struktur_cd = c.struktur_cd
                ...
                INNER JOIN trx_advance_process e ON d.process_id = e.process_id
                INNER JOIN surat_tugas f ON a.ref_id = f.spt_id
                WHERE b.item_uraian LIKE ? AND ((e.process_st = 'waiting' AND a.advance_status = 'waiting') OR a.advance_status = 'approved') AND a.group_id = '22'
                ORDER BY a.send_date DESC";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
    ...
    // get_list_advance_jaldin_lpj
    function get_list_advance_jaldin_lpj($params) {
        $sql = "SELECT b.item_uraian, c.struktur_nama, c.struktur_singkatan, a.advance_tanggal, a.advance_total_requested, a.trx_id, e.process_st, e.process_id, a.advance_total_approved, f.uraian_tugas
                FROM trx_advance a 
                INNER JOIN rencana_item b ON a.kode_item = b.kode_item
                INNER JOIN data_struktur_organisasi c ON a.struktur_cd = c.struktur_cd
    ...
    function get_trx_advance_by_process_id($params) {
        $sql = "SELECT a.*, b.group_name, c.kode_output, c.item_no, c.item_uraian, d.struktur_nama, d.struktur_singkatan, e.process_id, e.process_st, e.catatan, e.flow_id, f.uraian_tugas
                FROM trx_advance a
